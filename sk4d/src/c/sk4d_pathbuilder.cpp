...
void sk4d_pathbuilder_add_polygon(sk_pathbuilder_t* self, sk_point_t polygon[], int32_t count, bool is_closed) {
    ...
    AsPathBuilder(self)->addPolygon(SkSpan<const SkPoint>(AsPoint(polygon), safe_count), is_closed);
}
...
void sk4d_pathbuilder_polyline_to(sk_pathbuilder_t* self, const sk_point_t points[], int32_t count) {
    const size_t safe_count = count > 0 ? static_cast<size_t>(count) : 0;
    AsPathBuilder(self)->polylineTo(SkSpan<const SkPoint>(AsPoint(points), safe_count));
}
