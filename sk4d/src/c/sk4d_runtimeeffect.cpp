...
static sk_sp<SkImage> MakeRuntimeEffectImage(const SkRuntimeEffect* effect, GrRecordingContext* rContext, sk_sp<const SkData> uniforms, SkSpan<const SkRuntimeEffect::ChildPtr> children, const SkMatrix* localMatrix, SkImageInfo resultInfo, bool mipmapped) {
    sk_sp<SkSurface> surface;

    if (!effect ||
        (resultInfo.alphaType() == kUnpremul_SkAlphaType ||
    ...
    auto shader = effect->makeShader(std::move(uniforms), children, localMatrix);
...
sk_image_t* sk4d_runtimeeffect_make_image(const sk_runtimeeffect_t* self, gr_directcontext_t* context, const void* uniforms, sk_flattenable_t* children[], const sk_matrix_t* local_matrix, const sk_imageinfo_t* image_info, bool mipmapped) {
    ...
    return ToImage(MakeRuntimeEffectImage(AsRuntimeEffect(self), SK4D_ONLY_GPU(AsGrDirectContext(context), nullptr), SkData::MakeWithoutCopy(uniforms, AsRuntimeEffect(self)->uniformSize()), SkSpan(vector), (local_matrix) ? &m : nullptr, AsImageInfo(image_info), mipmapped).release());
}
...
sk_image_t* sk4d_runtimeshaderbuilder_make_image(sk_runtimeshaderbuilder_t* self, gr_directcontext_t* context, const sk_matrix_t* local_matrix, const sk_imageinfo_t* image_info, bool mipmapped) {
    ...
    if (local_matrix)
        m = AsMatrix(local_matrix);
    return ToImage(MakeRuntimeEffectImage(AsRuntimeShaderBuilder(self)->effect(), SK4D_ONLY_GPU(AsGrDirectContext(context), nullptr), AsRuntimeShaderBuilder(self)->uniforms(), AsRuntimeShaderBuilder(self)->children(), (local_matrix) ? &m : nullptr, AsImageInfo(image_info), mipmapped).release());
    return nullptr;
}
