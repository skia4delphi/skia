...
// Ganesh
    ...
    static inline GrContextOptions AsGrContextOptions(const gr_contextoptions_t* options) {
        ...
        static inline skgpu::VulkanYcbcrConversionInfo AsGrVkYcbcrConversionInfo(const gr_vk_ycbcrconversioninfo_t* info) {
            ...
            if (info->external_format != 0) {
                return skgpu::VulkanYcbcrConversionInfo(
                        info->external_format,
                        AsVkSamplerYcbcrModelConversion(info->ycbcr_model),
                        AsVkSamplerYcbcrRange(info->ycbcr_range),
                        AsVkChromaLocation(info->x_chroma_offset),
                        AsVkChromaLocation(info->y_chroma_offset),
                        AsVkFilter(info->chroma_filter),
                        info->force_explicit_reconstruction,
                        components,
                        info->format_features);
            }
            return skgpu::VulkanYcbcrConversionInfo(
                    AsVkFormat(info->format),
                    AsVkSamplerYcbcrModelConversion(info->ycbcr_model),
                    AsVkSamplerYcbcrRange(info->ycbcr_range),
                    AsVkChromaLocation(info->x_chroma_offset),
                    AsVkChromaLocation(info->y_chroma_offset),
