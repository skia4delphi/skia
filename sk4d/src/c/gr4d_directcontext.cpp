...
void gr4d_directcontext_flush_and_submit(gr_directcontext_t* self, bool sync_cpu) {
    SK4D_ONLY_GPU(
        AsGrDirectContext(self)->flush(GrFlushInfo());
        AsGrDirectContext(self)->submit(AsGrSyncCpu(sync_cpu));)
}
...
gr_directcontext_t* gr4d_directcontext_make_gl(const gr_gl_interface_t* gl_interface, const gr_contextoptions_t* options) {
    ...
    return SK4D_ONLY_GL(ToGrDirectContext(GrDirectContexts::MakeGL(sk_ref_sp(AsGrGLInterface(gl_interface)), (options) ? AsGrContextOptions(options) : defaultOptions).release()), nullptr);
}
...
gr_directcontext_t* gr4d_directcontext_make_metal(const gr_mtl_backendcontext_t* backend_context, const gr_contextoptions_t* options) {
    ...
    return SK4D_ONLY_METAL(ToGrDirectContext(GrDirectContexts::MakeMetal(AsGrMtlBackendContext(backend_context), (options) ? AsGrContextOptions(options) : defaultOptions).release()), nullptr);
}
...
gr_directcontext_t* gr4d_directcontext_make_vulkan(const gr_vk_backendcontext_t* backend_context, const gr_contextoptions_t* options) {
    ...
    return SK4D_ONLY_VULKAN(ToGrDirectContext(GrDirectContexts::MakeVulkan(AsGrVkBackendContext(backend_context), (options) ? AsGrContextOptions(options) : defaultOptions).release()), nullptr);
}
...
void gr4d_directcontext_purge_unlocked_resources(gr_directcontext_t* self, bool scratch_resources_only) {
    SK4D_ONLY_GPU(AsGrDirectContext(self)->purgeUnlockedResources((scratch_resources_only) ? GrPurgeResourceOptions::kScratchResourcesOnly : GrPurgeResourceOptions::kAllResources);)
}
...
bool gr4d_directcontext_submit(gr_directcontext_t* self, bool sync_cpu) {
    return SK4D_ONLY_GPU(AsGrDirectContext(self)->submit(AsGrSyncCpu(sync_cpu)), false);
}
