...
void sk4d_canvas_draw_atlas(sk_canvas_t* self, const sk_image_t* atlas, const sk_rotationscalematrix_t transforms[], const sk_rect_t sprites[], const sk_color_t colors[], int32_t count, sk_blendmode_t blend_mode, const sk_samplingoptions_t* sampling, const sk_rect_t* cull_rect, const sk_paint_t* paint) {
    ...
    AsCanvas(self)->drawAtlas(AsImage(atlas),
                              SkSpan<const SkRSXform>(AsRotationScaleMatrix(transforms), safe_count),
                              SkSpan<const SkRect>(AsRect(sprites), safe_count),
                              SkSpan<const SkColor>(colors, colors ? safe_count : 0),
                              AsBlendMode(blend_mode), AsSamplingOptions(*sampling),
                              AsRect(cull_rect), AsPaint(paint));
}
...
void sk4d_canvas_draw_glyphs(sk_canvas_t* self, int32_t count, const sk_glyphid_t glyphs[], const sk_point_t positions[], const sk_point_t* origin, const sk_font_t* font, const sk_paint_t* paint) {
    const size_t safe_count = count > 0 ? static_cast<size_t>(count) : 0;
    AsCanvas(self)->drawGlyphs(SkSpan<const SkGlyphID>(glyphs, safe_count),
                               SkSpan<const SkPoint>(AsPoint(positions), safe_count),
                               *AsPoint(origin), AsFont(*font), AsPaint(*paint));
}
...
void sk4d_canvas_draw_glyphs2(sk_canvas_t* self, int32_t count, const sk_glyphid_t glyphs[], const sk_rotationscalematrix_t matrices[], const sk_point_t* origin, const sk_font_t* font, const sk_paint_t* paint) {
    const size_t safe_count = count > 0 ? static_cast<size_t>(count) : 0;
    AsCanvas(self)->drawGlyphsRSXform(
            SkSpan<const SkGlyphID>(glyphs, safe_count),
            SkSpan<const SkRSXform>(AsRotationScaleMatrix(matrices), safe_count),
            *AsPoint(origin), AsFont(*font), AsPaint(*paint));
}
...
void sk4d_canvas_draw_points(sk_canvas_t* self, sk_drawpointsmode_t mode, size_t count, const sk_point_t points[], const sk_paint_t* paint) {
    AsCanvas(self)->drawPoints(AsDrawPointsMode(mode),
                               SkSpan<const SkPoint>(AsPoint(points), count),
                               AsPaint(*paint));
}
...
sk_surface_t* sk4d_canvas_make_surface(sk_canvas_t* self, const sk_imageinfo_t* image_info, const sk_surfaceprops_t* surface_props) {
    ...
    if (surface_props)
        SkSurfaceProps props = AsSurfaceProps(surface_props);
    return ToSurface(AsCanvas(self)->makeSurface(AsImageInfo(image_info), (surface_props) ? &props : nullptr).release());
}
