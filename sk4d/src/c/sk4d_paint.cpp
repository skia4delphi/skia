...
sk_path_t* sk4d_paint_get_fill_path(const sk_paint_t* self, const sk_path_t* path, const sk_rect_t* cull_rect, float res_scale) {
    ...
    const bool is_fill = skpathutils::FillPathWithPaint(
            AsPath(*path), AsPaint(*self), &builder, AsRect(cull_rect),
            SkMatrix::Scale(res_scale, res_scale));
    return is_fill ? ToPath(new SkPath(builder.detach())) : nullptr;
}
