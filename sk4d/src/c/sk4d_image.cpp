...
bool sk4d_image_is_valid(const sk_image_t* self, gr_directcontext_t* context) {
    SK4D_ONLY_GPU(auto recorder = context ? AsGrDirectContext(context)->asRecorder() : skcpu::Recorder::TODO();)
    return AsImage(self)->isValid(SK4D_ONLY_GPU(recorder, skcpu::Recorder::TODO()));
}
...
sk_image_t* sk4d_image_make_subset(const sk_image_t* self, const sk_irect_t* subset, gr_directcontext_t* context) {
    SK4D_ONLY_GPU(auto recorder = context ? AsGrDirectContext(context)->asRecorder() : nullptr;)
    return ToImage(AsImage(self)->makeSubset(
            SK4D_ONLY_GPU(recorder, nullptr), AsIRect(*subset), {}).release());
}
