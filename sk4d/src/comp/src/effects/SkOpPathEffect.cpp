...
bool SkOpPE::onFilterPath(SkPathBuilder* dst, const SkPath& src, SkStrokeRec* rec,
                          const SkRect* cull, const SkMatrix& ctm) const {
    SkPath one, two;
    if (fOne) {
        SkPathBuilder builder;
        if (!fOne->filterPath(&builder, src, rec, cull, ctm)) {
            return false;
        }
        one = builder.detach();
    } else {
    ...
    if (fTwo) {
        SkPathBuilder builder;
        if (!fTwo->filterPath(&builder, src, rec, cull, ctm)) {
            return false;
        }
        two = builder.detach();
    } else {
...
bool SkMatrixPE::onFilterPath(SkPathBuilder* dst, const SkPath& src, SkStrokeRec*, const SkRect*,
                              const SkMatrix&) const {
    *dst = src.makeTransform(fMatrix);
    return true;
}
...
sk_sp<SkPathEffect> SkStrokePathEffect::Make(SkScalar width, SkPaint::Join join, SkPaint::Cap cap,
                                             SkScalar miter) {
    if (!SkIsFinite(width, miter) || width < 0 || miter < 0) {
        return nullptr;
    }
...
bool SkStrokePE::onFilterPath(SkPathBuilder* dst, const SkPath& src, SkStrokeRec*, const SkRect*,
                              const SkMatrix&) const {
...
bool SkStrokeAndFillPE::onFilterPath(SkPathBuilder* dst, const SkPath& src, SkStrokeRec* rec,
    ...
    if (rec->getStyle() == SkStrokeRec::kStroke_Style) {
        ...
        if (!rec->applyToPath(&stroked_builder, src)) {
            return false;
        }
        SkPath stroked = stroked_builder.detach();
        *dst = stroked;

        if (known_to_be_opposite_directions(src, stroked)) {
            SkPathPriv::ReverseAddPath(dst, src);
        } else {
            dst->addPath(src);
        }
    } else {
